...
infected2 <- as.data.frame(cbind(dateseq, infectedseq))
...
epicurve2 <- infected %>% 
  full_join(recovered, by =c('onset' = 'recovered'))
...
epicurve2 <- epicurve2 %>% 
  ...
  mutate(total_infected = cumsum(n_infected))

#Calculate number of people not susceptible and susceptible 
epicurve2 <- epicurve2 %>% 
  mutate(susceptible = 154 - total_infected)
```

***Plot susceptible, infected, and recovered by day***
```{r}
plot2 <- 
  ggplot(epicurve2,aes(x=onset), labs = FALSE) +
           geom_line(aes(y = susceptible), color = "red") +
           geom_line(aes(y = n_infected), color = "blue") +
           geom_line(aes(y = total_recovered), color = "green") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  labs(x = "Date",
       y = "Number of Cases",
       title = "Epi Curve for Rollins Fall Fever")
