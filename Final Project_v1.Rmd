...
linelist <- linelist %>% 
	mutate(recovered2 = case_when((symptomatic == 0 | is.na(symptomatic)) ~ exposure0 + hours(asymp_duration), #imputed infectious periods for asymptomatic or missing
															 ...
															 TRUE ~ NA)) %>% 
	mutate(recovered = date(recovered2))
```
...
Severe_cases <- linelist %>% 
	filter(severe == 1)
>>>>>>> 44618dd899fcb39e65293a1198d4d19aaa87a3d2
...
>>>>>>> 44618dd899fcb39e65293a1198d4d19aaa87a3d2
```{r}
mean_incubation <- symptomatic %>%
	summarise(mean(incubation_period, na.rm = T))
#23.36

sd_incubation <- symptomatic %>% 
	summarise(sd(incubation_period, na.rm = T))
#7.38
```

<<<<<<< HEAD
Non-Severe Symptomatic (Natural History)
=======
...
#Incubation period for Severe cases and Non_severe cases
```{r}
mean(Severe$incubation_period)
#23.43

sd(Severe$incubation_period)
#6.38

mean(Non_Severe$incubation_period)
#23.30

sd(Non_Severe$incubation_period)
#8.43
```
```{r}
...
linelist <- linelist %>% 
  rowwise() %>% 
  mutate(infectiousness_end = pmax(exposure1, exposure2, exposure3, exposure4, exposure5,recovered2, severe_date, na.rm = TRUE))
