...
```{r}
#Create new onset variable using date and time of exposure + incubation period among symptomatic individuals

linelist <- linelist %>% 
	mutate(onset = case_when(symptomatic == 0 ~ NA,
													 symptomatic == 1 ~ exposure0 + hours(incubation_period),
													 is.na(symptomatic) ~ NA,
													 TRUE ~ NA)) %>% 
  mutate(onset = date(onset))

#Create new recovered variable using date and time of exposure + incubation period + illness duration for symptomatic individuals, or the severity date, if after that time

linelist <- linelist %>% 
	mutate(recovered = case_when(symptomatic == 0 ~ NA,
															 (symptomatic == 1 & severe == 1) ~ pmax(onset + hours(illness_duration), severe_date),
...
```{r}
infected <- linelist %>%
	filter(symptomatic == 1) %>% #not sure if we need to rethink this curve only among symptomatic people?
  group_by(exposure0) %>%
  summarize(n_infect = n()) %>%
  na.omit()
